...
```bash
INIT=$(node "C:/1Wes/all-in-badminton-app/.codex/get-shit-done/bin/gsd-tools.cjs" init plan-phase "$PHASE")
...
```bash
PHASE_INFO=$(node "C:/1Wes/all-in-badminton-app/.codex/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}")
```
...
```bash
node "C:/1Wes/all-in-badminton-app/.codex/get-shit-done/bin/gsd-tools.cjs" state planned-phase --phase "${PHASE}" --name "${phase_name}" --plans "${PLAN_COUNT}"
```
