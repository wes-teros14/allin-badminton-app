...
- **Global scope**: `C:/1Wes/all-in-badminton-app/.codex/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/1Wes/all-in-badminton-app/.codex/notes/`)
...
2. Glob `C:/1Wes/all-in-badminton-app/.codex/notes/*.md` (if directory exists) — global notes
...
Global (C:/1Wes/all-in-badminton-app/.codex/notes/):
...
2. **No `.planning/`**: Falls back to global `C:/1Wes/all-in-badminton-app/.codex/notes/` — works in any directory
