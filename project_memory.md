...
- **Eligibility, both boards**: 3+ sessions attended (`MIN_SESSIONS_PLAYED`, read from `player_stats.sessions_attended`), plus registered in at least one of the last 4 completed sessions. The Partners board additionally needs 3 games together, and applies *both* player rules to *both* partners — one regular carrying a newcomer must not mint a top-ten pairing (FR-014a).
...
- The Cheers tab still uses the older `RANK_ICON` helper. That is intentional — those lists are top-five *counts*, not places.
