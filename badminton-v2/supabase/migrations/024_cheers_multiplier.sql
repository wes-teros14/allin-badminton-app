-- =============================================================
-- Migration: 024_cheers_multiplier
-- Re-adds unique constraint (1 cheer per giver-receiver per session).
-- Adds multiplier column so a single cheer counts N times in stats,
-- where N = games played together in that session.
-- =============================================================

-- Re-add unique constraint
ALTER TABLE public.cheers
  ADD CONSTRAINT cheers_session_id_giver_id_receiver_id_key
  UNIQUE (session_id, giver_id, receiver_id);

-- Add multiplier (must be >= 1)
ALTER TABLE public.cheers
  ADD COLUMN multiplier INT NOT NULL DEFAULT 1
  CHECK (multiplier >= 1);

-- Update stats trigger to multiply counts by multiplier
